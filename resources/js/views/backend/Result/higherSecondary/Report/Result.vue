    ...
    <div class="card border">
      <div class="card-header">
        <div class="row">
          ...
          <div v-if="Object.keys(data.details).length > 0" class="col-6">
            ...
            <router-link
              :to="{ name: 'result.marksheetAll', params: { id: data.id } }"
              class="btn btn-xs btn-primary float-end me-3"
            >
              <i class="bx bx-list-ul"></i> ALL MARKSHEET
            </router-link>

            <a
              class="btn btn-xs btn-info float-end me-3"
              @click="downloadBulkMarksheet()"
              href="javascript:;"
            >
          ...
          <table class="table table-bordered table-hover table-striped mb-0">
            ...
            >
              <tr v-for="(std, key) in data.details" :key="key">
                ...
                <td>
                  <router-link
                    :to="{ name: 'result.marksheet', params: { id: std.id } }"
                    target="_blank"
                  >
                    {{ std.student_id }}
                  </router-link>
...
export default {
      ...
      this.$validate().then((res) => {
        if (res) {
          ...
          let params = {
            ...
            type: this.type,
            failed_subject: this.failed_subject,
            field_name: this.field_name,
            search_keyword: this.search_keyword,
          };

          axios
            .get("/result-report", { params: params })
            .then((res) => {
              if (res.status == 200) {
      ...
      this.$validate().then((res) => {
        if (res) {
          ...
          if (confirm("Are you sure want to download?")) {
            ...
            let url = `${this.$root.baseurl}/admin/download-bulk-marksheet?search_params=${jsonStr}`;
  ...
  async created() {
    this.setBreadcrumbs(this.model, "view", "Result", addOrBack);
