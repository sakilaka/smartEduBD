...
export default {
    ...
    search() {
      ...
      this.$validate().then((res) => {
        if (res) {
          ...
          axios
            .get("/primaryResult-report", { params: params })
            .then((res) => {
              if (res.status == 200) {
      ...
      this.$validate().then((res) => {
        if (res) {
          ...
          if (confirm("Are you sure want to download?")) {
            ...
            let url = `${this.$root.baseurl}/admin/primary-download-bulk-marksheet?search_params=${jsonStr}`;
  ...
  created() {
    this.setBreadcrumbs(this.model, "view", "Primary Result", addOrBack);
