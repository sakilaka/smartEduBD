          ...
          <div class="row">
            ...
            <div class="col-6 text-end">
              <button
                type="button"
                v-if="
                  $root.checkPermission('primaryResult.syncResult') &&
                  this.subject_id
                "
              ...
              <button
                type="button"
                v-if="$root.checkPermission('primaryResult.syncResult')"
...
export default {
  ...
  methods: {
    ...
    marksCalculate(key, field = null) {
      ...
      let ct = mark.ct_mark ? mark.ct_mark : 0;
      let cq = mark.cq_mark ? mark.cq_mark : 0;
      let mcq = mark.mcq_mark ? mark.mcq_mark : 0;

      let obtainedMak = parseFloat(cq) + parseFloat(mcq) + parseFloat(ct);
      let examTotalMark = parseFloat(this.subject.total_mark);
      let obtained = (parseFloat(obtainedMak) / examTotalMark) * 100;

      let gradeGpa = this.letterGradeGpa(obtained, mark);

      mark["obtained_mark"] = obtained.toFixed(2);
      mark["total_mark"] = obtained.toFixed(2);
    ...
    getStudents() {
      this.$validate().then((res) => {
        ...
        axios
          .get("/primary-students-for-marks-entry", { params: params })
          .then((res) => {
  ...
  async created() {
    ...
    this.setBreadcrumbs(this.model, page, "Primary Result", addOrBack);
    ...
    this.get_data("primaryGradeManagement", "letter_grades", {
      allData: true,
    });
  },
