...
class SecondaryResultReportController extends Controller
{
    use ResultTrait;
    ...
    public function result(Request $request)
    {
        ...
        $query->with(['details' => function ($q) use ($search) {
            $q->select(
                'secondary_result_details.*',
                'std.software_id',
                'std.name_en',
                'profile.roll_number'
            )->join('students as std', 'secondary_result_details.student_id', '=', 'std.id')
                ->join('student_profiles as profile', 'std.id', '=', 'profile.student_id');

            $q->with(['marks' => function ($mq) {
                $mq->with('subject')->select('secondary_result_details_id', 'subject_id')->where('letter_grade', 'F');
            }]);
            ...
            if (in_array($search['type'],  [
                ...
                $q->where('result_status', 'PASSED')
                    ->orderBy("secondary_result_details.{$search['type']}", 'asc');
                // ->orderBy('secondary_result_details.gpa', 'desc')
    ...
    public function classwiseResult(Request $request)
    {
        ...
        $query->with(['details' => function ($q) use ($search) {
            ...
            $q->where('result_status', 'PASSED')
                ->orderBy("secondary_result_details.merit_position_in_class", 'asc');
        }]);
    ...
    public function subjectwiseResult(Request $request)
    {
        ...
        $query->with(['details' => function ($q) use ($search) {
            $q->select(
                'secondary_result_details.id',
                'secondary_result_details.secondary_result_id',
                'secondary_result_details.student_id',
                'rm.*',
                'std.id',
                'std.software_id',
                'std.name_en',
                ...
                ->join('students as std', 'secondary_result_details.student_id', '=', 'std.id')
                ->join('student_profiles as profile', 'std.id', '=', 'profile.student_id')
                ->join('secondary_result_marks as rm', 'secondary_result_details.id', '=', 'rm.secondary_result_details_id')
    ...
    {
        return SecondaryResultDetails::with(['marks' => function ($q) {
    ...
    {
        $detail = SecondaryResultDetails::with(
            'student',
            'result.institution',
            'result.academic_session',
            'result.institution',
        ...
        $pdf      = Pdf::loadView('pdf.secondary_marksheet', compact('detail', 'storage_url', 'highest_marks'))->setPaper('a4', 'landscape');
    ...
    public function downloadBulkMarksheet(Request $request)
    {
        ...
        $query->with(['details' => function ($q) use ($type) {
            ...
            if (in_array($type,  [
                ...
                $q->where('result_status', 'PASSED')
                    ->orderBy("secondary_result_details.{$type}", 'asc');
                // ->orderBy('secondary_result_details.gpa', 'desc')
                // ->orderBy('total_mark_without_additional', 'desc')
                // ->orderBy('profile.roll_number', 'asc');
        ...
        $pdf      = Pdf::loadView('pdf.secondary_marksheet_bulk', compact('result', 'storage_url', 'highest_marks'))->setPaper('a4', 'landscape');
        $fileName = "marksheet.pdf";
        // return $pdf->download($fileName);
        return $pdf->stream();
    }
    ...
    private function subjectLists($request)
    {
        ...
        if (!empty($subjectAssign)) {
            $subjects = $subjectAssign->details()
                ->select('secondary_subject_assign_id', 'subject_id')
    ...
    public function gradeSummary(Request $request)
    {
        ...
        $subject_summary = DB::table('secondary_result_marks as rm')
            ->selectRaw('
                    rm.subject_id,
                ...
                ')
            ->join('secondary_result_details as rd', 'rm.secondary_result_details_id', '=', 'rd.id')
            ->join('subjects as sub', 'rm.subject_id', '=', 'sub.id')
            ->where('rd.secondary_result_id', $result->id)
    ...
    public function numberSheet(Request $request)
    {
        ...
        $data['result'] = SecondaryResult::with(
            'academic_session',
            'institution',
            'campus',
            'shift',
